...
public class LingoesLd2Reader {
	...
	public static void main(final String[] args) throws IOException {
		...
		// String ld2File =
		// Helper.DIR_IN_DICTS+"\\lingoes\\Prodic English-Vietnamese Business.ld2";
		final String ld2File = "E:/prestoWord/词库/Oxford Advanced Learner's Dictionary.ld2";
	...
	private static final long decompress(final String inflatedFile,
	...
	private static final SensitiveStringDecoder[] detectEncodings(
	...
	private static final void extract(final String inflatedFile,
			final String indexFile, final String extractedWordsFile,
			final String extractedXmlFile, final String extractedOutputFile,
			final int[] idxArray, final int offsetDefs, final int offsetXml)
		...
		final ByteBuffer dataRawBytes = ByteBuffer.allocate((int) fChannel
				.size());
		fChannel.read(dataRawBytes);
		fChannel.close();
		dataRawBytes.order(ByteOrder.LITTLE_ENDIAN);
		...
		final SensitiveStringDecoder[] encodings = LingoesLd2Reader
				.detectEncodings(dataRawBytes, offsetDefs, offsetXml, defTotal,
	...
	private static final void getIdxData(final ByteBuffer dataRawBytes,
	...
	private static final void inflate(final ByteBuffer dataRawBytes,
	...
	private static final void readDefinitionData(
			final ByteBuffer inflatedBytes, final int offsetWords,
	...
	private static final void readDictionary(final String ld2File,
			final ByteBuffer dataRawBytes, final int offsetWithIndex)
			throws IOException, FileNotFoundException,
			UnsupportedEncodingException {
		...
		if (new File(inflatedFile).isFile()) {
			...
			LingoesLd2Reader.extract(inflatedFile, indexFile, extractedFile,
					extractedXmlFile, extractedOutputFile, idxArray,
					inflatedWordsIndexLength, inflatedWordsIndexLength
							+ inflatedWordsLength);
		}
	}
	...
	private static final void writeInputStream(final InputStream in,
